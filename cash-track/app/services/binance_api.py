...
class BinanceIntegration:
    ...
    def save_credentials(user_id: int, api_key: str, api_secret: str, is_testnet: bool = False) -> bool:
        ...
        try:
            ...
            cursor.execute('SELECT id FROM binance_credentials WHERE user_id = %s', (user_id,))
            ...
            if existing:
                ...
                cursor.execute('''
                    ...
                    SET api_key = %s, api_secret = %s, is_testnet = %s, updated_at = CURRENT_TIMESTAMP
                    WHERE user_id = %s
    ...
    def delete_credentials(user_id: int) -> bool:
        ...
        try:
            conn = get_db()
            cursor = conn.cursor()
            cursor.execute('DELETE FROM binance_credentials WHERE user_id = %s', (user_id,))
            conn.commit()
            conn.close()
            return True
        except Exception as e:
